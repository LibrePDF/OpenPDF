...
public class FSLinearGradient {
    ...
    public FSLinearGradient(final FSFunction func, final CalculatedStyle style, final int width, final int height,
        ...
        for (int j = 1; j < stopPoints.size(); j++) {
            if (j + 1 < stopPoints.size() && stopPoints.get(j).dotsValue == null && increment == 0f) {
                int k = j + 1;
        ...
        for (int b = 0; b < stopPoints.size() - 1; b++) {
            if (stopPoints.get(b).dotsValue.equals(stopPoints.get(b + 1).dotsValue)) {
                // Duplicate lengths.
                constructZero();
                return;
            }
