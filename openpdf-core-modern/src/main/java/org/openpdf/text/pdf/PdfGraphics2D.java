...
import org.openpdf.text.utils.SystemPropertyUtil;
...
*/
    ...
    private boolean drawImage(Image img, Image mask, AffineTransform xform, Color bgColor, ImageObserver obs) {
        ...
        try {
            org.openpdf.text.Image image = null;
            if (!convertImagesToJPEG) {
                image = org.openpdf.text.Image.getInstance(img, bgColor);
            } else {
                ...
                scaled.flush();
                scaled = null;
                image = org.openpdf.text.Image.getInstance(baos.toByteArray());

            }
            if (mask != null) {
                org.openpdf.text.Image msk = org.openpdf.text.Image.getInstance(mask, null, true);
                msk.makeMask();
    ...
    private void setPaint(boolean invert, double xoffset, double yoffset, boolean fill) {
        if (paint instanceof Color color) {
            ...
            try {
                ...
                org.openpdf.text.Image image = org.openpdf.text.Image.getInstance(img, null);
    ...
    private static class CompositeFontDrawer {
         ...
         *                               {@link org.openpdf.text.pdf.BaseFont base font}
