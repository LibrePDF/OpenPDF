...
public class DvdCover {
    ...
    public static void main(String[] args) {
        System.out.println("Objects :: Images :: DVD Cover");
        DvdCover cover = new DvdCover();
        if (args.length > 1) {
            cover.setFilename(args[1]);
        }
        if (args.length > 2) {
            cover.setTitle(args[2]);
        }
        if (args.length > 3) {
            cover.setBackgroundcolor(Color.decode(args[3]));
        }
        if (args.length > 4) {
            try {
                cover.setFront(Image.getInstance(DvdCover.class.getClassLoader().getResource(args[4])));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        if (args.length > 5) {
            try {
                cover.setBack(Image.getInstance(DvdCover.class.getClassLoader().getResource(args[5])));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        if (args.length > 6) {
            try {
                cover.setSide(Image.getInstance(DvdCover.class.getClassLoader().getResource(args[6])));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        cover.generatePdf(args);
    }
