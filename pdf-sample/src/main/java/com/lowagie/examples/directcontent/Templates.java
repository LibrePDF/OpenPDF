 ...
 */
public class Templates extends AbstractSample {
    ...
    public void render(String path) {
        ...
        System.out.println("DirectContent :: Templates");
        ...
        try (Document document = new Document()) {
            ...
            PdfTemplate template = cb.createTemplate(500, 200); // <1>

            // we add some graphics
            template.moveTo(0, 200);
            template.lineTo(500, 0);  // <2>
            template.stroke();
            template.setRGBColorStrokeF(255f, 0f, 0f);
            template.circle(250f, 100f, 80f);  // <3>
            template.stroke();
            ...
            template.showText("Text at the position 100,100 (relative to the template!)"); // <4>
            template.endText();
            template.sanityCheck();

            // we add the template on different positions
            cb.addTemplate(template, 0, 0); // <5>
