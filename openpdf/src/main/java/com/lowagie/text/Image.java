 ...
 public abstract class Image extends Rectangle {
     ...
     public static Image getInstance(URL url) throws BadElementException, IOException {
         ...
         try {
             ...
             if (isGifFormat(byte1, byte2, byte3)) {
                 img = ImageLoader.getGifImage(url);
                 return img;
             }
             if (isJpegFormat(byte1, byte2)) {
                 img = ImageLoader.getJpegImage(url);
                 return img;
             }
             if (isJpeg2000Format(byte1, byte2, byte3, byte4)) {
                 img = ImageLoader.getJpeg2000Image(url);
                 return img;
             }
             if (isPngFormat(byte1, byte2, byte3, byte4)) {
                 img = ImageLoader.getPngImage(url);
                 return img;
             }
             if (isBmpFormat(byte1, byte2)) {
                 img = ImageLoader.getBmpImage(url);
                 return img;
             }
             if (isTiffFormat(byte1, byte2, byte3, byte4)) {
                 img = ImageLoader.getTiffImage(url);
                 return img;
             }
             if (isJbig2Format(byte1, byte2, byte3, byte4, byte5, byte6, byte7, byte8)) {
                 throw new IOException(url.toString()
                         + " is not a recognized image format. JBIG2 support has been removed.");
             }
             throw new IOException(url.toString()
     ...
     private static boolean isPngFormat(int byte1, int byte2, int byte3, int byte4) {
         return byte1 == PNGID[0] && byte2 == PNGID[1] && byte3 == PNGID[2] && byte4 == PNGID[3];
     }
     ...
     private static boolean isJbig2Format(int byte1, int byte2, int byte3, int byte4,
             int byte5, int byte6, int byte7, int byte8) {
         return byte1 == 0x97 && byte2 == 'J' && byte3 == 'B' && byte4 == '2' &&
                 byte5 == '\r' && byte6 == '\n' && byte7 == 0x1A && byte8 == '\n';
     }
      ...
      *
      * @param image the <CODE>java.awt.Image</CODE> to convert
      * @param color if different from <CODE>null</CODE> the transparency pixels are
